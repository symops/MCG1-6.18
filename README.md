...
Porting Linux **v6.18.49** to the WD My Cloud (Gen 1) NAS.
...
- No public mainline support for this SoC has ever existed. The
  ...
  SoC). This repository forward-ports that work to v6.18.49 and
...
above) -- that's what makes the release string a clean `6.18.49`
matching the module directory (`/lib/modules/6.18.49/`) installed
above.
...
boots, now running under v6.18.49. (Plain `make uImage` still works
...
- One squashed baseline commit importing `linux-stable` v6.18.49
  ...
  tag `v6.18.49`, for anyone who needs to trace a specific change).
